...
function Library:CreateWindow(config)
    ...
    local openScale = Library:Create("UIScale", {Scale = 1, Parent = openBtn})
    ...
    function Library:ApplyResponsiveLayout()
        pcall(function()
            ...
            if Library._genieBusy then return end
    ...
    local function makeChromeIconBtn(btnPosX, makeGlyph, baseBg, baseTrans)
        baseBg = baseBg or Library.Theme.CardBg
        if baseTrans == nil then baseTrans = 1 end
        local hoverTrans = math.max(0, baseTrans - 0.4)
        ...
        btn.MouseEnter:Connect(function()
            Library:Tween(btn, {BackgroundTransparency = hoverTrans}, 0.12):Play()
            setGlyphColor(Library.Theme.Text)
        end)
        btn.MouseLeave:Connect(function()
            Library:Tween(btn, {BackgroundTransparency = baseTrans}, 0.12):Play()
    ...
    -- Minimize: ขีด — พื้นหลังเทาอ่อนจางๆ
    local MIN_BG = Color3.fromRGB(120, 120, 135)
    ...
    -- ── Genie minimize/restore (macOS-style): หุบเข้าหาปุ่มลอย ──
    -- dest = openBtn.Position/Size ตรงๆ (หน่วยเดียวกันทั้งคู่) → ทับปุ่มพอดี ไม่หลุดจอ
    -- tween แค่ Position+Size (ไม่แตะ mainScale กันบีบ 2 ต่อ, ไม่ใส่ Rotation กันของทะลุกรอบ)
    -- cover กระดาษเปล่าคลุมเนื้อหาก่อนหุบ + การ์ดกัน responsive กระชากกลาง tween
    local MinAnimToken = 0
    local savedWinPos, savedWinSize = nil, nil
    local genieBusy = false
    Library._genieBusy = false

    -- cover กระดาษเปล่า: fade-in 0.15 วิให้เนื้อหากลายเป็นพื้นเปล่าก่อนหุบ
    local GenieCover = Library:Create("Frame", {
        Name                   = "GenieCover",
        Size                   = UDim2.new(1, 0, 1, 0),
        BackgroundColor3       = Library.Theme.MainBg,
        BackgroundTransparency = 1,
    ...
    Library:Create("UICorner", {CornerRadius = UDim.new(0, 10), Parent = GenieCover})

    local function GenieHide()
        MinAnimToken = MinAnimToken + 1
        local token = MinAnimToken
        if not MainFrame.Visible or genieBusy then return end
        genieBusy = true
        Library._genieBusy = true
        savedWinPos  = MainFrame.Position
        savedWinSize = MainFrame.Size
        pcall(function()
            local sp = MainFrame:FindFirstChild("SettingsPanel")
            if sp then sp.Visible = false end
            Library.SettingsOpen = false
        end)

        -- ขั้น 1: กระดาษเปล่า (fade cover ขึ้นคลุมเนื้อหา) + ซ่อนไส้ในทันที
        GenieCover.Visible = true
        Library:Tween(GenieCover, {BackgroundTransparency = 0}, 0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out):Play()
        pcall(function()
            for _, n in ipairs({"TopBar", "SideBar", "ContentArea"}) do
                local f = MainFrame:FindFirstChild(n)
                if f then f.Visible = false end
            end
        end)

        -- เตรียมปุ่มลอยแบบซ่อนไว้ก่อน (ย่อเหลือ 30% + ไอคอนโปร่งใส) — ค่อยโชว์ตอนหุบเสร็จ ไม่โผล่เป็นจุดดำกลางอนิเมชัน
        openBtn.Visible = false
        local openIcon = openBtn:FindFirstChildOfClass("ImageLabel")
        local obFull = (Library.TouchEnabled and 56 or 44)
        openScale.Scale = 0.3
        if openIcon then openIcon.ImageTransparency = 1 end

        task.delay(0.16, function()
            if token ~= MinAnimToken or Library.Unloaded then genieBusy = false Library._genieBusy = false return end
            -- ขั้น 2: หุบกระดาษเปล่าเข้าหาปุ่มลอย (Position/Size หน่วยเดียวกัน ลงจอดทับปุ่มพอดี)
            -- ไม่ใส่ Rotation: ไม่งั้น ClipsDescendants พัง ของข้างในทะลุออกนอกกรอบ
            local destPos  = openBtn.Position
            local destSize = UDim2.new(0, math.max(40, obFull), 0, math.max(40, obFull))
            local downT = 0.42
            Library:Tween(MainFrame, {Position = destPos, Size = destSize}, downT, Enum.EasingStyle.Quart, Enum.EasingDirection.In):Play()
            task.delay(downT, function()
                if token ~= MinAnimToken or Library.Unloaded then genieBusy = false Library._genieBusy = false return end
                MainFrame.Visible = false
                MainFrame.Position = savedWinPos
                MainFrame.Size = savedWinSize
                GenieCover.BackgroundTransparency = 1
                GenieCover.Visible = false
                -- โชว์ปุ่มลอยตอนหุบเสร็จแล้ว (เด้ง pop + ไอคอน fade-in) — ไม่โผล่กลางอนิเมชัน
                openBtn.Visible = true
                local popT = 0.32
                Library:Tween(openScale, {Scale = 1}, popT, Enum.EasingStyle.Back, Enum.EasingDirection.Out):Play()
                if openIcon then
                    Library:Tween(openIcon, {ImageTransparency = 0}, 0.2):Play()
                end
                task.delay(popT + 0.05, function()
                    if token ~= MinAnimToken or Library.Unloaded then genieBusy = false Library._genieBusy = false return end
                    openScale.Scale = 1
                    openBtn.Size = UDim2.new(0, obFull, 0, obFull)
                    if openIcon then openIcon.ImageTransparency = 0 end
                    genieBusy = false
                    Library._genieBusy = false
                end)
            end)
    ...
    local function GenieShow()
        MinAnimToken = MinAnimToken + 1
        local token = MinAnimToken
        if MainFrame.Visible or genieBusy then
            openBtn.Visible = false
            return
        end
        genieBusy = true
        Library._genieBusy = true
        local homePos  = savedWinPos  or MainFrame.Position
        local homeSize = savedWinSize or MainFrame.Size
        local obFull = (Library.TouchEnabled and 56 or 44)

        -- เริ่มจากจุดปุ่มลอย (เล็ก) แล้วกางออก — ไส้ในยังซ่อนอยู่จนกางเสร็จ
        local openIcon = openBtn:FindFirstChildOfClass("ImageLabel")
        MainFrame.Position = openBtn.Position
        MainFrame.Size = UDim2.new(0, math.max(40, obFull), 0, math.max(40, obFull))
        GenieCover.Visible = true
        GenieCover.BackgroundTransparency = 0
        MainFrame.Visible = true
        openBtn.Visible = false

        local upT = 0.45
        Library:Tween(MainFrame, {Position = homePos, Size = homeSize}, upT, Enum.EasingStyle.Back, Enum.EasingDirection.Out):Play()
        task.delay(0.25, function()
            if token ~= MinAnimToken or Library.Unloaded then return end
            Library:Tween(GenieCover, {BackgroundTransparency = 1}, 0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out):Play()
        end)
        task.delay(upT + 0.05, function()
            if token ~= MinAnimToken or Library.Unloaded then genieBusy = false Library._genieBusy = false return end
            MainFrame.Position = homePos
            MainFrame.Size = homeSize
            GenieCover.BackgroundTransparency = 1
            GenieCover.Visible = false
            pcall(function()
                for _, n in ipairs({"TopBar", "SideBar", "ContentArea"}) do
    ...
    Library.GenieHide = GenieHide
    Library.GenieShow = GenieShow

    MinBtn.MouseButton1Click:Connect(function()
        GenieHide()
    end)

    -- Close: กากบาท — พื้นหลังแดงอ่อนจางๆ
    local CLOSE_BG = Color3.fromRGB(220, 65, 65)
    ...
    CloseBtn.MouseEnter:Connect(function()
        Library:Tween(CloseBtn, {
            ...
            BackgroundColor3 = Color3.fromRGB(90, 22, 28),
        }, 0.12):Play()
        setCloseGlyphColor(Color3.fromRGB(255, 255, 255))
    end)
    CloseBtn.MouseLeave:Connect(function()
        Library:Tween(CloseBtn, {
            BackgroundTransparency = 0.72,
            BackgroundColor3 = CLOSE_BG,
        }, 0.12):Play()
    ...
    -- กดที่การ์ด = ก็อปปี้ Username (ปุ่มโปร่งใสคลุมทั้งการ์ด)
    local ProfileHit = Library:Create("TextButton", {
        Name                   = "CopyUsername",
        Size                   = UDim2.new(1, 0, 1, 0),
        BackgroundTransparency = 1,
        Text                   = "",
    ...
    Library:Create("UICorner", {CornerRadius = UDim.new(0, 8), Parent = ProfileHit})
    ProfileHit.MouseEnter:Connect(function()
        Library:Tween(ProfileCard, {BackgroundColor3 = Library.Theme.CardHoverBg}, 0.12):Play()
    end)
    ProfileHit.MouseLeave:Connect(function()
        Library:Tween(ProfileCard, {BackgroundColor3 = Library.Theme.CardBg}, 0.12):Play()
    end)
    ProfileHit.MouseButton1Click:Connect(function()
        ...
        pcall(function()
            if copied then
                Library:Notify({ Title = "Copied Username", Content = "@" .. userName, Duration = 2, Type = "Success" })
            else
                Library:Notify({ Title = "Copy Failed", Content = userName, Duration = 2, Type = "Error" })
            end
        end)
    end)
