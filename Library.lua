...
function Library:CreateWindow(config)
    ...
    -- เก็บ geometry เดิม → tween Position/Size/Rotation เข้าหาปุ่ม → ค่อยซ่อน
    -- ขากลับ: โผล่จากปุ่ม → tween กลับ + fade/shear คืน — กัน spam ด้วย token
    ...
    local function GenieHide()
        ...
        local tbCX = MainFrame.AbsolutePosition.X + mfSize.X / 2
        local obCX = openBtn.AbsolutePosition.X + openBtn.AbsoluteSize.X / 2
        local mfX = MainFrame.Position.X.Offset
        local openW = openBtn.AbsoluteSize.X
        local dx = (obCX - tbCX) / s
        local destX = mfX + dx - (mfSize.X - (openW / s)) / 2
        local destPos  = UDim2.new(MainFrame.Position.X.Scale, destX, MainFrame.Position.Y.Scale, MainFrame.Position.Y.Offset)
        local destSize = UDim2.new(0, math.max(40, openW / s), 0, math.max(28, openBtn.AbsoluteSize.Y / s))

        -- เตรียมปุ่มลอยให้โผล่ "ใต้" หน้าต่างที่กำลังหุบ + เซ็ตให้เล็ก/โปร่งใสก่อน
        ...
        if openScale then
            ...
            openBtn.Size = UDim2.new(0, obFull * 0.3, 0, obFull * 0.3)
        end
        pcall(function()
            for _, d in ipairs(openBtn:GetDescendants()) do
                if d:IsA("ImageLabel") or d:IsA("TextButton") then
        ...
        Library:Tween(mainScale, {Scale = math.max(0.02, s * 0.12)}, downT, Enum.EasingStyle.Quart, Enum.EasingDirection.In):Play()
        task.delay(downT, function()
            ...
            if openScale then
                ...
                Library:Tween(openBtn, {Size = UDim2.new(0, obFull, 0, obFull)}, popT, Enum.EasingStyle.Back, Enum.EasingDirection.Out):Play()
            end
            pcall(function()
                for _, d in ipairs(openBtn:GetDescendants()) do
                    if d:IsA("ImageLabel") or d:IsA("TextButton") then
                        Library:Tween(d, {ImageTransparency = 0, TextTransparency = 0}, 0.2):Play()
                    end
                end
            end)
            task.delay(popT + 0.05, function()
                if token ~= MinAnimToken or Library.Unloaded then return end
                if openScale then openScale.Scale = 1 end
    ...
    local function GenieShow()
        ...
        local mfSize = MainFrame.AbsoluteSize
        local tbCX = openBtn.AbsolutePosition.X + openBtn.AbsoluteSize.X / 2
        local obCX = homePos.X.Offset + (mfSize.X * s) / 2
        local mfX = homePos.X.Offset
        local dx = (tbCX - obCX) / s
        MainFrame.Position = UDim2.new(homePos.X.Scale, mfX + dx, homePos.Y.Scale, homePos.Y.Offset)
        MainFrame.Size = UDim2.new(0, math.max(40, obFull / s), 0, math.max(28, obFull / s))
        MainFrame.Rotation = -4
        mainScale.Scale = math.max(0.02, s * 0.12)
        ...
        Library:Tween(mainScale, {Scale = s}, upT, Enum.EasingStyle.Back, Enum.EasingDirection.Out):Play()
